...
- `renderer.textureGC.maxIdle` is lowered to `TEXTURE_GC_IDLE_SEC` (30 s) so atlases nobody has drawn recently release their GPU copy; PIXI re-uploads from the retained bitmap on the next draw.
...
- **Memory policy on touch devices** (`GameConfig.mobile`, resolved in `constants.ts` via `isTouchDevice`): `getRenderScale()` is capped at what the screen can physically show (`dpr × scale-to-fit`, from `screen` dims so a portrait load or URL bar can't bake in a low value) — that one cap flows into the canvas backing store, `atlasScaleFor` and every fitted skin; atlas headroom drops 1.25 → `atlasHeadroom` (1.0); `lowMemoryAtlasScale` shrinks atlases further when `navigator.deviceMemory ≤ 4`; `gpuOnlyAtlases` turns `ATLAS_GPU_ONLY` on (single GPU copy) paired with `textureGcIdleSec` 600 so nothing churns. Desktop keeps the original behaviour — there the resolution setting doubles as supersampling.
- **Two invariants in `SpriteRegistry` that are about memory, not correctness.** (1) `drawAtlas` uses a CPU canvas (`willReadFrequently: true`): sheets are software bitmaps (decoded in PIXI's worker), and drawing one into an *accelerated* canvas pushes all 37 MB through a GPU transfer buffer that Chrome pools at its high-water mark — measured at ~600 MB of never-returned `gpu/mapped_memory` after one match start. (2) `loadLayerAnim` is throttled (`SHEET_LOAD_CONCURRENCY` = 3) because a match start otherwise decodes ~40 sheets at once (~1.4 GB transient — tab-kill territory on a 4 GB phone). Together they took the phone-emulation renderer from ~890 MB to ~340 MB; Start → match still ≈ 1.4 s.
