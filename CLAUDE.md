...
- `renderer.textureGC.maxIdle` is `TEXTURE_GC_IDLE_SEC` (600 s). Atlases are **GPU-only** everywhere (`GameConfig.spriteAtlasGpuOnly` → `ATLAS_GPU_ONLY`): the bitmap is closed after upload, so an eviction means rebuilding the atlas from its sheet — hence the long idle; within a match nothing is evicted and match switches unload sets explicitly.
- Map skins (background, ground, decor, platform/block, tower templates, coin overrides, coin box) load through `loadFittedTexture(url, logicalW, logicalH, worldSpace)` (`SkinTextures.ts`): decoded once via `Assets`, shrunk to `logical × GAME_ZOOM(world only) × getRenderScale() × 1.25` when the PNG is larger than that, cached per URL, full-size decode unloaded. Tiled skins fit only when both tile dimensions are explicit (an unspecified axis tiles at the image's natural size). `Game.reset()` diffs `collectMapAssetUrls()` before/after and evicts the previous map's leftovers via `unloadSkinTextures`. Shared defaults (coin PNGs, coin-box skin, power-up art) are never in that set.
- Dev perf panel: **TEX** = resident sprite-atlas bytes (`spriteTextureBytes()`), **GPU** = every base texture currently holding a GL texture (`Game.gpuTextureBytes()` over `renderer.texture.managedTextures`).
- **Texture scale ≠ render scale** (`resolution.ts`). `getRenderScale()` is the resolution setting (capped at what the screen can show on touch devices — `dpr × scale-to-fit` from `screen` dims). `getTextureScale()` = `min(renderScale, displayScale)` and is what sizes **textures** (`atlasScaleFor`, `SkinTextures`): a sprite drawn into a supersampled backing store still lands on the same device pixels, so texels beyond the display are memory with no picture in it. The renderer keeps its supersampling; atlases don't pay for it.
- **Lazy body anims.** `carry` and `throw` measured 34 % of atlas bytes and only play after a coin pickup, so the eager per-set load skips them (`LAZY_BODY_ANIMS`); `Character.showCoinCarry()` calls `requestLazyBodyAnims`, and `switchBodyAnimation` does too whenever it falls back because one is missing. Frames are spliced into the live `LoadedSpriteSet`; `tickAnimSprite` treats "lazy anim arrived" (`currentBodyAnim !== bodyAnimResolved`) as a change so the stand-in is replaced. A lazy load that lands after `unloadSpriteSetsExcept` evicted its set is discarded.
- **Touch-device knobs** (`GameConfig.mobile`): `atlasHeadroom` 1.0 (desktop 1.25) and `lowMemoryAtlasScale` when `navigator.deviceMemory ≤ 4`.
- **Two invariants in `SpriteRegistry` that are about memory, not correctness.** (1) `drawAtlas` uses a CPU canvas (`willReadFrequently: true`): sheets are software bitmaps (decoded in PIXI's worker), and drawing one into an *accelerated* canvas pushes all 37 MB through a GPU transfer buffer that Chrome pools at its high-water mark — measured at ~600 MB of never-returned `gpu/mapped_memory` after one match start. (2) `loadLayerAnim` is throttled (`SHEET_LOAD_CONCURRENCY` = 3) because a match start otherwise decodes ~40 sheets at once (~1.4 GB transient — tab-kill territory on a 4 GB phone).
- Measured on a live match (Chrome memory-infra dump): game renderer 900 → 338 MB desktop, 888 → 329 MB phone emulation; resident atlases 444 → 167 / 559 → 207 MB; Start → match ≈ 1.9 s.
- **Startup budget.** Map skins live as files under `public/maps/skins/` (content-addressed), not as data URLs in `defaultMapData.json`: after every "Bake defaultMapData.json" run `npm run bake:maps` (`scripts/extract-map-skins.mjs`), which rewrites the data URLs to paths — the JSON went 3.6 MB → 13 KB and left the main JS bundle. Audio loads on the **first unmute**, not at startup (the game starts muted; `AudioManager.initAudio` is idempotent and called from `setMuted(false)`), and `GameConfig.audio.sounds` lists real files — every entry is one variant, no URL probing. The Google Fonts stylesheet is loaded non-blocking (`media="print" onload`).
- **DOM / HUD art is stored near its displayed size** (≈ 2× the CSS size for DPR headroom), not at authoring resolution: cards 384 px (shown ≤ 300 px), power-up icons and `command_hud` 320 px (44–134 px), coins 256 px (40 px HUD / ~60 px in-world), `timer` 192 px, `box_01` 512 px, splash hero 2048 px served as WebP. The originals were 741–4417 px: ~70 MB decoded and ~1.2 MB of transfer for images shown at thumbnail size, and on slow-4G they delayed the splash hero by ~6 s. When adding UI art, size it to where it is drawn; in-world skins go through `loadFittedTexture` and are shrunk at load, but still pay full transfer + first decode.
